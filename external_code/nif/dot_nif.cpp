...
//to compile
// g++ -O3 -fpic -shared  -o ./priv/dot_nif.so -I $ERL_ROOT/usr/include ./external_code/nif/dot_nif.cpp
using namespace std;
...
inline bool fillVector(ErlNifEnv* env, ERL_NIF_TERM listTerm, vector<double> &result) 
{
    ...
    for (unsigned int i = 0; i < length; ++i) {
        ...
        currentList = tail;
        if (!enif_get_double(env, head, &actualHead)) { // prende l'elemento della lista (è double, scorre tutta la lista)
            return false;
        }
...
ERL_NIF_INIT(Elixir.ElixirCpp, nif_funcs, nullptr, nullptr, nullptr, nullptr);